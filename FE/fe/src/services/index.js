...
export async function verifyVnpayPaymentService(vnpayData) {
    const { data } = await axiosInstance.post(`/payment/vnpay/verify-return`, vnpayData);
...
export async function sendOTPResetPasswordService(userEmail) {
    const { data } = await axiosInstance.post("/auth/send-otp-reset", {
        userEmail,
    });
...
export async function resetPasswordService(userEmail, otpCode, otpToken, newPassword) {
